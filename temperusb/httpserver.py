...
class HTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        for dev in self.devs:
            temperatures = dev.get_temperatures(sensors=self.sensors,detach=False)
            humidities = dev.get_humidity(sensors=self.sensors,detach=False)
            combinations = {}
        ...
        for i, reading in enumerate(readings):
            ...
            if self.quiet:
                ...
                huminfo = ''
                for sensor in sorted(reading):
                    if self.args.disp_ports and portinfo == '':
...
def parse_args():
    ...
    parser.add_argument("-P", "--http_port", type=int,
